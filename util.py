...
def backup_database():
    while True:
        ...
        shutil.copy(os.path.join(root, 'database.db'), os.path.join(backup_folder, f'{timestamp}.db'))
        ...
        for backup in os.listdir(backup_folder):
            ...
            if (datetime.datetime.now() - backup_time).days > get_config('max_backup_days'):
